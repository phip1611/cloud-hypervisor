...
mod common_parallel {
    ...
    fn _test_live_migration_guest_event(guest_event: GuestEventDuringMigration) {
        ...
        // The stress workload scales with the guest memory.
        let guest = basic_regular_guest!(JAMMY_IMAGE_NAME).with_memory("1G");
        let src_api_socket = temp_api_path(&guest.tmp_dir);
        ...
        let r = panic::catch_unwind(panic::AssertUnwindSafe(|| {
            ...
            // Detach the command as the SSH helper re-runs it after a lost
            // connection, which would then hit the destination.
            ...
            guest
                .ssh_command(&format!(
                    "nohup sudo sh -c 'sleep 1; {guest_command}' &>/dev/null &"
                ))
                .unwrap();
