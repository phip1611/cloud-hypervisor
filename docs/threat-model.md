...
therefore has the same power as a management interface client.
...
the built-in mutual TLS support must be used, or the UNIX socket
transport combined with filesystem permissions. This also covers the
additional migration connections that a TCP migration accepts on the
same listener.
