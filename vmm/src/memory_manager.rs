...
impl MemoryManager {
    ...
    pub fn create_ram_region_raw(
        ...
        // Must happen before prefaulting: with the host policy set to
        // `madvise`, only a region that is already marked as eligible gets
        // huge pages when its pages are faulted in.
        if thp && !hugepages {
            // SAFETY: FFI call with correct arguments
