...
//! # Asynchronous Migration Worker (Coordinator)
//!
//! Data types and functionality to drive the migration asynchronously in
//! the background. The main export of the module is [`MigrationWorker`]. Each
//! migration is driven by exactly one worker/coordinator.
//!
//! ## Migration Worker Lifecycle
//!
...
///
/// Each migration is driven by exactly one worker/coordinator.
pub struct MigrationWorker {
    ...
    /// Eventfd used to signal the VMM thread that the migration thread
    /// terminated (success, cancellation, or failure).
