...
use crate::migration::worker::{MigrationThreadOut, MigrationWorker, MigrationWorkerHandle};
...
pub enum EpollDispatch {
    ...
    Debug = 4,
    GuestExit = 5,
    /// Indicates the VMM should check the outcome of the migration thread,
    /// which is either success, cancellation, or failure.
    CheckMigration = 6,
    Unknown,
}
...
/// Describes the current ownership of a VM and all associated resources
/// with that state.
pub enum VmOwnership {
    ...
    Migration {
        /// Handle to the [`MigrationWorker`].
        migration_worker_handle: MigrationWorkerHandle,
        /// Latest VM info response snapshot. This stays constant during a
        /// migration as the VM is immutable (regarding its virtual hardware).
        vm_info_response: VmInfoResponse,
    },
    /// No VM is running or (temporarily) not marked as owned by the VMM.
    None,
}

impl VmOwnership {
    /// Takes the VM from the enum and replaces the ownership with
    /// [`Self::None`].
    ///
    /// # Panics
    /// This method panics if `self` is not [`Self::Vmm`].
    fn take_vm(&mut self) -> Option<Vm> {
        match mem::replace(self, Self::None) {
            VmOwnership::Vmm(vm) => Some(vm),
            old => {
                *self = old;
                None
            }
    ...
    /// Returns the optional mutable reference to the underlying VM.
    ///
    /// This only fails when the VM is currently owned by the migration worker.
    fn vm_mut_or_none(&mut self) -> result::Result<Option<&mut Vm>, VmError> {
        match self {
            VmOwnership::Vmm(vm) => Ok(Some(vm)),
            VmOwnership::Migration { .. } => Err(VmError::VmMigrating),
            VmOwnership::None => Ok(None),
        }
    }
    ...
    fn vm_mut(&mut self, none_err: VmError) -> result::Result<&mut Vm, VmError> {
        self.vm_mut_or_none()?.ok_or(none_err)
    }
}
...
impl Vmm {
    ...
    fn send_migration(
        ...
        let mut socket = transport::send_migration_socket(&send_data_migration.destination_url)?;
    ...
    /// Checks the result of the migration thread.
    ///
    /// When the migration thread exits (because of success, cancellation, or
            ...
            for event in events.iter().take(num_events) {
                ...
                match dispatch_event {
                    ...
                    EpollDispatch::ActivateVirtioDevices => {
                        if let VmOwnership::Vmm(ref vm) = self.vm {
                    ...
                    EpollDispatch::Debug => {
                        ...
                        for _ in 0..self.debug_evt.read().map_err(Error::EventFdRead)? {
                            ...
                            let response = if let VmOwnership::Vmm(ref mut vm) = self.vm {
...
impl RequestHandler for Vmm {
    ...
    fn vm_restore(&mut self, restore_cfg: RestoreConfig) -> result::Result<(), VmError> {
        if self.vm.vm_mut_or_none()?.is_some() {
            return Err(VmError::VmAlreadyCreated);
        }
    ...
    fn vm_info(&self) -> result::Result<VmInfoResponse, VmError> {
        ...
        let state = match &self.vm {
            VmOwnership::Vmm(vm) => vm.get_state(),
        ...
        let device_tree = match &self.vm {
            VmOwnership::Vmm(vm) => Some(vm.device_tree().lock().unwrap().clone()),
            VmOwnership::None => None,
            VmOwnership::Migration { .. } => unreachable!("migration path is handled above"),
        };
    ...
    fn vm_resize(
        ...
        if let Some(vm) = self.vm.vm_mut_or_none()? {
    ...
    fn vm_receive_migration(
        ...
        let mut listener =
            transport::receive_migration_listener(&receive_data_migration.receiver_url)?;
